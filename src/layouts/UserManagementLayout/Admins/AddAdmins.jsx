...
const AddAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validationErrors === true) {
      ...
      try {
        await axios.post("http://192.168.109.81/iCorner/api/user", adminData);
        toast.success("Admin added successfully!"); // Show success toast
        setTimeout(() => {
          navigate("/users/view-admins");
        }, 2000); // Delay navigation to allow the toast to show
      } catch (error) {
        console.error("Error adding admin:", error);
        toast.error("Error adding admin. Please try again."); // Show error toast
      } finally {
  ...
  return (
    <Container className="add-admin-container mt-4">
      {/* Back Arrow */}
      <div className="mb-4">
        <Button
          variant="link"
      ...
      <h2 className="text-center mb-4 main-heading">Add Admin</h2>
      <Row>
          ...
          <Form onSubmit={handleSubmit}>
            ...
            <div className="text-center">
              <Button className="submit-btn" type="submit" disabled={loading}>
                {loading ? "Adding..." : "Add Admin"} {/* Change button text */}
              </Button>
            </div>
          </Form>
        </Col>
